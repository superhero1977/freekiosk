...
const TABS: { id: string; label: string; icon: IconName }[] = [
  ...
  { id: 'security', label: '安全', icon: 'shield-lock' },
  { id: 'advanced', label: '高级', icon: 'cog' },
];
...
const SettingsScreenNew: React.FC<SettingsScreenProps> = ({ navigation }) => {
  ...
  const handlePickMediaFromDevice = async (type: 'video' | 'image' | 'any') => {
    try {
      ...
      if (error?.code !== 'PICKER_CANCELLED') {
        Alert.alert('错误', `Failed to pick media: ${error?.message || error}`);
      }
    } finally {
  ...
  const loadInstalledApps = async (): Promise<void> => {
    try {
      ...
      Alert.alert('错误', `Unable to load apps: ${error}`);
    } finally {
  ...
  const handleCheckForUpdates = async () => {
    ...
    try {
      ...
      Alert.alert('错误', `Unable to check for updates: ${error.message || error.toString()}`);
    } finally {
  ...
  const handleDownloadUpdate = async (update?: any) => {
    ...
    try {
      ...
      if (!canInstall) {
        Alert.alert(
          '⚠️ 需要权限',
          'FreeKiosk 需要权限才能安装更新。\n\n请在下一个界面上启用"允许来自此来源"，然后返回并重试更新。',
          [
            { text: 'Cancel', style: 'cancel' },
            {
              text: 'Open Settings',
    ...
    try {
      ...
      if (error?.code === 'INSTALL_PERMISSION' || errorMsg.includes('unknown sources')) {
        Alert.alert(
          '⚠️ 需要安装权限',
          '更新已下载但无法安装。\n\n请在设备设置中为 FreeKiosk 启用"允许安装未知来源应用"，然后重试。\n\n在受限设备上（例如 Echo Show），请使用：\nadb install -r <apk>',
        );
      } else {
        Alert.alert('错误', `Download failed:\n\n${errorMsg}`);
      }
    }
  };
  ...
  const handleSave = async (): Promise<void> => {
    ...
    if (displayMode === 'external_app') {
      if (externalAppMode === 'single') {
        ...
        if (!regex.test(externalAppPackage)) {
          Alert.alert('错误', '包名格式无效（例如 com.example.app）');
          return;
        }
        try {
          const isInstalled = await AppLauncherModule.isAppInstalled(externalAppPackage);
          if (!isInstalled) {
            Alert.alert('错误', `App not installed: ${externalAppPackage}`);
            return;
          }
        } catch (error) {
          Alert.alert('错误', `Unable to verify app: ${error}`);
          return;
        }
      } else {
        ...
        if (homeScreenApps.length === 0) {
          Alert.alert('错误', 'Multi App mode requires at least one app with "在主屏幕显示" enabled');
          return;
        }
      }
    }
    ...
    if (displayMode === 'webview' && !dashboardModeEnabled) {
      ...
      if (urlLower.startsWith('file://') || urlLower.startsWith('javascript:') || urlLower.startsWith('data:')) {
        Alert.alert('安全错误', '不允许此类型的网址。请使用 http:// 或 https://');
        return;
      }
    ...
    if (pinModeChanged && !pin) {
      Alert.alert('错误', 'Password mode changed - you must enter a new password');
      return;
    }
    ...
    if (pin && pin.length > 0) {
      ...
      if (pinMode === 'numeric' && !/^\d+$/.test(pin)) {
        Alert.alert('错误', 'In numeric PIN mode, only digits (0-9) are allowed. Enable "高级密码模式" to use letters and special characters.');
        return;
      }
      if (pinMode === 'numeric' && pin.length > 6) {
        Alert.alert('错误', 'Numeric PIN must be 4-6 digits. Enable "高级密码模式" for longer passwords.');
        return;
      }
    } else if (!isPinConfigured && !pin) {
      Alert.alert('错误', 'Please enter a password');
      return;
    }
    ...
    if (isNaN(inactivityDelayNumber) || inactivityDelayNumber <= 0) {
      Alert.alert('错误', 'Please enter a valid inactivity delay');
      return;
    }
    ...
    if (isNaN(pinMaxAttemptsNumber) || pinMaxAttemptsNumber < 1 || pinMaxAttemptsNumber > 100) {
      Alert.alert('错误', 'PIN attempts must be between 1 and 100');
      return;
    }
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('成功', 'Settings reset!\nPlease reconfigure the app.', [
                { text: 'OK', onPress: () => { revokeSettingsAccess(); navigation.reset({ index: 0, routes: [{ name: 'Kiosk' }] }); } },
              ]);
            } catch (error) {
              Alert.alert('错误', `Reset failed: ${error}`);
            }
          },
        },
      ],
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('错误', `Unable to exit: ${error}`);
            }
          },
        },
      ],
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('错误', `Failed: ${error.message || error}`);
            }
          },
        },
      ],
        ...
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('成功', 'Certificate removed');
            } catch (error) {
              Alert.alert('错误', `Failed: ${error}`);
            }
          },
        },
      ],
