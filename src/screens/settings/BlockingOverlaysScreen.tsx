...
const BlockingOverlaysScreen: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const handleShowTouchLogger = async () => {
    try {
      ...
      if (!success) {
        Alert.alert('错误', '显示触摸记录器失败');
      }
    } catch (error) {
      Alert.alert('错误', '显示触摸记录器失败. Make sure overlay permission is granted.');
    }
  };
  ...
  const handleShowGridHelper = async () => {
    try {
      ...
      if (!success) {
        Alert.alert('错误', '显示网格辅助失败');
      }
    } catch (error) {
      Alert.alert('错误', '显示网格辅助失败. Make sure overlay permission is granted.');
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.title}>遮挡叠加层</Text>
      </View>
